...
from flux_led.BulbScanner import BulbScanner,INFOFIELD_IPADDR, INFOFIELD_ID, INFOFIELD_MODEL
from flux_led.WifiLedBulb import WifiLedBulb
...
def parseArgs():
    ...
    other_group.add_option("-d", "--debug", 
    ...
    parser.usage = "usage: %prog [-sS10cwpCiltThed] [addr1 [addr2 [addr3] ...]."
...
def main():
    ...
    if options.scan:
        ...
        if options.scanresults and len(bulb_info_list) > 0 :
            for b in bulb_info_list:
                addrs.append(b[INFOFIELD_IPADDR])
        else:
            print("{} bulbs found:".format(len(bulb_info_list)))
            for b in bulb_info_list:
                print("-  id={} ip={} model={}".format(b[INFOFIELD_ID], b[INFOFIELD_IPADDR],b[INFOFIELD_MODEL]))
            sys.exit(0)
        ...
        for addr in args:
            info = dict()
            info[INFOFIELD_IPADDR] = addr
            info[INFOFIELD_ID] = 'Unknown ID'
    ...
    for info in bulb_info_list:
        try:
            bulb = WifiLedBulb(info[INFOFIELD_IPADDR])
        except Exception as e:
            print("Unable to connect to bulb at [{}]: {}".format(info[INFOFIELD_IPADDR],e))
            continue

        if options.getclock:
            print("{} [{}] {}".format(info[INFOFIELD_ID], info[INFOFIELD_IPADDR],bulb.getClock()))
        ...
        if options.info:
            bulb.update_state()
            print("{} [{}] {}".format(info[INFOFIELD_ID], info[INFOFIELD_IPADDR], bulb))
